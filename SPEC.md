...
- **Dimensionality: 6,102.** The number of parameters in a genome. This is the figure quoted throughout the document, and it is what determines search cost.
- **Cardinality: about 10^14,695.** The number of distinct genomes, at a reference quantisation of 8 bits per parameter: `2^(8 × 6,102) = 2^48,816 ≈ 10^14,695`.

For scale, the observable universe contains on the order of **10^80** atoms. The genome space is larger by a factor of about 10^14,615.

The figure moves with the quantisation assumed and should always be quoted with it: at 10 bits per parameter it is ~10^18,369, at ~6.6 bits (roughly 100 distinguishable values each) ~10^12,123. Every such figure is an over-count of *audibly distinct* Creatures, because the genome→sound map is many-to-one by design (§3.1) — but the over-count is a rounding error at these magnitudes.

Even the **expressed** subspace at initialisation — ~163 parameters — is about 10^393 at 8 bits, still unenumerable by many hundreds of orders of magnitude.
...
| **Point** — one specific genome | **No** | any particular 6,102-parameter Creature |
...
The genome declares ~6,100 parameters. At initialisation roughly **163** affect the output. The remainder is neutral material — muted waves, unused envelope nodes — which mutates freely at zero fitness cost and remains available for reactivation.
...
**64 wave slots × 95 genes + 22 global = 6,102 parameters.**
...
`fundamental_cents`, `tempo_bpm` (30 – 300), `master_gain`, `sigma_global`, `p_duplicate`, `p_switch_flip_scale`, `n_partners` (1 – 8, init 1.4), `partner_influence` (0 – 0.5, init 0.15), `mutation_fraction` (0 – 1), plus **14 visualiser genes** (§11).
...
Final output is `Σ out[i]`, peak-normalised, scaled by `master_gain`.
...
d_global = mean |gene_A − gene_B| over the 22 global genes, stored [0,1] space
...
Global genes are ignored in this calculation; they are 22 of 6,102 parameters and folding them in changes nothing at three significant figures.
...
| Inputs | full genome (6,102, stored space) + hour-of-day (sin/cos) + listener id (one-hot) + session index + position in session + current `L` |
...
**State is persisted to IndexedDB, not `localStorage`.** A full genome is 6,102 floats — roughly 24 KB raw — and a run of several thousand listens will exceed the ~5 MB `localStorage` quota within the first few hundred. IndexedDB quotas are typically a large fraction of free disk. Storage schema and volume management are specified in §14.
...
**Dwell** — `dwell_s`, `completed`, `unattended`, `discarded_short`, `visibility_pause_count`, `visibility_paused_ms`, `focus_pause_count`, `focus_paused_ms`, `idle_triggered`.
...
`cells_occupied`, `coverage` (occupied / 256), a **histogram of cell depths** (how many cells hold 1…8 residents), and the sum of cell mean fitnesses as a QD-score equivalent.
...
On every evaluation of the servo, whether or not it acts: `old_L`, `new_L` (or `no_change`), `direction`, `p_completed`, `k`, `L_prime_computed`, `window_n`, the **full sorted dwell vector for the window**, and `suppressed_by_5pct_guard`.
...
Gates 2a and 2b produce tables, not runtime logs, and are written once per run as separate files: the locality curve (median perceptual distance against ε, per gene class), the `J_class` table, the compatibility-distance distribution with the §6.5 worked intuitions beside the measured values, and the behavioural-locality results (`p_same`, `p_near`, `H_cell`, mutation-only and with-crossover).

### 14.5 Anomalies

A separate stream: render failures, NaN or infinity in synthesis, clipping, cycle-detection results (`n_back_edges`), storage-quota events, and any case where a guard fired (cooldown exhausted its 5 attempts, protection blocked every candidate for eviction, partner sampling returned the prime parent).

### 14.6 What the logs must be able to answer
...
GLOBAL_GENES            = 22        GENOME_SIZE         = 6102
P_ACTIVE_AT_INIT        = 0.03      MIN_ACTIVE          = 1
EXPRESSED_AT_INIT       ≈ 163       SEED_BATCH          = 32   (σ = 0.2)
...
TAU_GLOBAL              = 0.0091    TAU_LOCAL   = 0.0801
