...
- **Scored SSE:** 696.526  (lower is better; 0 = a perfect, sample-identical match)
- **Similarity (1/SSE):** 0.001436
- **Silence floor for this target:** 7975.962 — a silent render scores this. This render is **11.451× better than silence**.
- **MIMIC's best on this target:** 7943.730 — ARTISAN is **11.405× better**.
...
- **Wall-clock time:** 14285.6 s.
- **Random seed:** 1 (reproducible).

### Budget spent (anytime optimiser)

The optimiser ran 41 portfolio epochs and was still descending when the budget was reached (it did **not** self-terminate — more budget would buy more). Marginal gain over the last third of the run: 2.38e+1 SSE per 1.27 h = 1.88e+1 SSE/hour. The engine-vs-fast-model gap at the end was 5.13e-7 (the additive fast scorer stayed faithful to the true engine).
