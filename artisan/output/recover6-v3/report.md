...
- **Scored SSE:** 9.610  (lower is better; 0 = a perfect, sample-identical match)
- **Similarity (1/SSE):** 0.104056
- **Silence floor for this target:** 4583.688 — a silent render scores this. This render is **476.960× better than silence**.
- **MIMIC's best on this target:** 130.920 — ARTISAN is **13.623× better**.
...
- **Wall-clock time:** 1819.6 s.
- **Random seed:** 1 (reproducible).

### Budget spent (anytime optimiser)

The optimiser ran 8 portfolio epochs and was still descending when the budget was reached (it did **not** self-terminate — more budget would buy more). Marginal gain over the last third of the run: 1.11e+0 SSE per 0.07 h = 1.50e+1 SSE/hour. The engine-vs-fast-model gap at the end was 3.90e-8 (the additive fast scorer stayed faithful to the true engine).
