...
- **Scored SSE:** 190.812  (lower is better; 0 = a perfect, sample-identical match)
- **Similarity (1/SSE):** 0.005241
- **Silence floor for this target:** 407.248 — a silent render scores this. This render is **2.134× better than silence**.
...
- **Wall-clock time:** 1806.5 s.
- **Random seed:** 1 (reproducible).

### Budget spent (anytime optimiser)

The optimiser ran 8 portfolio epochs and was still descending when the budget was reached (it did **not** self-terminate — more budget would buy more). Marginal gain over the last third of the run: 2.29e+0 SSE per 0.06 h = 3.81e+1 SSE/hour. The engine-vs-fast-model gap at the end was 1.56e-7 (the additive fast scorer stayed faithful to the true engine).
