...
- **Scored SSE:** 9.963  (lower is better; 0 = a perfect, sample-identical match)
- **Similarity (1/SSE):** 0.100369
- **Silence floor for this target:** 4583.688 — a silent render scores this. This render is **460.062× better than silence**.
- **MIMIC's best on this target:** 130.920 — ARTISAN is **13.140× better**.
...
- **Wall-clock time:** 3603.2 s.
- **Random seed:** 1 (reproducible).

### Budget spent (anytime optimiser)

The optimiser ran 16 portfolio epochs and was still descending when the budget was reached (it did **not** self-terminate — more budget would buy more). Marginal gain over the last third of the run: 5.92e-1 SSE per 0.26 h = 2.30e+0 SSE/hour. The engine-vs-fast-model gap at the end was 2.86e-8 (the additive fast scorer stayed faithful to the true engine).
