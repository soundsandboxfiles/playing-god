...
- **Scored SSE:** 783.999  (lower is better; 0 = a perfect, sample-identical match)
- **Similarity (1/SSE):** 0.001276
- **Silence floor for this target:** 7975.962 — a silent render scores this. This render is **10.173× better than silence**.
- **MIMIC's best on this target:** 7943.730 — ARTISAN is **10.132× better**.
...
- **Wall-clock time:** 5434.4 s.
- **Random seed:** 1 (reproducible).

### Budget spent (anytime optimiser)

The optimiser ran 12 portfolio epochs and was still descending when the budget was reached (it did **not** self-terminate — more budget would buy more). Marginal gain over the last third of the run: 4.09e+1 SSE per 0.35 h = 1.17e+2 SSE/hour. The engine-vs-fast-model gap at the end was 7.13e-7 (the additive fast scorer stayed faithful to the true engine).
